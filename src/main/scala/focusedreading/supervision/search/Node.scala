...
/**
  ...
  */
case class Node(state:FRSearchState, action:Option[FocusedReadingAction], parent:Option[Node], depth:Int) extends Ordered[Node] {
  ...
  def pathCost = state.cost//state.agent.papersRead.size
}